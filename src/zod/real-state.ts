...
export const realStateSchema = z.object({
  ...
  square_meters: z.number().positive('Square meters must be a positive number').max(100000, 'Square meters must be at most 100,000.'),
  price: z.number().positive('Price must be a positive number').max(90000000, 'Price must be at most $90,000,000').multipleOf(0.01),
  currency: z.enum(['HNL', 'USD', 'EUR']).optional().default('HNL'),
